...
const getPaymentBookingsById = async (req,res) => {
    try {
        const {booking_id } = req.params;
        const booking = await flexiBooking.findOne({ _id: booking_id }).populate('payment_id');
        if(!booking){
            return res.status(404).json({message: "No bookings found"});
        }
        return booking;
...
const updateBookingById = async (req,res) =>{
    try {
        ...
        const updateBooking = await flexiBooking.findByIdAndUpdate(booking_id, req.body, { new: true})
        if(!updateBooking){
            return res.status(404).json({message: "Booking not found"});
        }
...
const deleteBookingById = async (req,res) =>{
    try {
        ...
        const deletedBooking = await flexiBooking.findByIdAndDelete({ _id: booking_id });
        if(!deletedBooking){
            return res.status(404).json({ message: "Booking not found" });
        }
