...
const bookingSchema = Joi.object({
  ...
  guest_checkin_status: Joi.boolean()
    .default(false)
    .optional(),
  
  guest_assign_desk: Joi.string()
    .optional(),
  
  identification_info: Joi.string()
    .required(),
...
const bookingUpdateSchema = Joi.object({
  ...
  const bookingUpdateSchemaValidation = async (req,res,next) => {
    try {
        ...
        logger.error("bookingUpdateSchemaValidation ::  user data is invalid", error);
        return res.status(500).send({ message: error.message });
      }
  }
...
const createPaymentSchemaValidation = async (req,res,next) => {
    try {
        ...
        logger.error("createPaymentSchemaValidation ::  user data is invalid", error);
        return res.status(500).send({ message: error.message });
      }
  }
  ...
  const updatePaymentSchema = Joi.object({
    ...
    const updatePaymentSchemaValidation = async (req,res,next) => {
        try {
            ...
            logger.error("updatePaymentSchemaValidation ::  user data is invalid", error);
            return res.status(500).send({ message: error.message });
          }
      }
  

module.exports = {bookingSchemaValidation,bookingUpdateSchemaValidation,createPaymentSchemaValidation,updatePaymentSchemaValidation};
